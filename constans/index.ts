import {
  mobile,
  backend,
  creator,
  web,
  javascript,
  typescript,
  html,
  css,
  reactjs,
  redux,
  tailwind,
  nodejs,
  mongodb,
  git,
  figma,
  docker,
  dicoding,
  bintang,
  gundar,
  nafas,
  eundang,
  bintangphoto,
  threejs,
  mendelof,
  forum,
  chat,
  monitor,
  react,
  node,
  sablonkuy,
} from '@/public/assets';

export const navLinks = [
  {
    id: 'home',
    title: '// Home',
  },
  {
    id: 'about',
    title: '// About',
  },
  {
    id: 'experiences',
    title: '// Experiences',
  },
  {
    id: 'contact',
    title: '// Contact',
  },
];

const services = [
  {
    title: 'Frontend Developer',
    icon: web,
  },
  {
    title: 'Backend Developer',
    icon: backend,
  },
  {
    title: 'React Native Developer',
    icon: mobile,
  },
  {
    title: 'Fullstack Developer',
    icon: creator,
  },
];

const technologies = [
  {
    name: 'HTML 5',
    icon: html,
  },
  {
    name: 'CSS 3',
    icon: css,
  },
  {
    name: 'JavaScript',
    icon: javascript,
  },
  {
    name: 'TypeScript',
    icon: typescript,
  },
  {
    name: 'React JS',
    icon: reactjs,
  },
  {
    name: 'Redux Toolkit',
    icon: redux,
  },
  {
    name: 'Tailwind CSS',
    icon: tailwind,
  },
  {
    name: 'Node JS',
    icon: nodejs,
  },
  {
    name: 'MongoDB',
    icon: mongodb,
  },
  {
    name: 'Three JS',
    icon: threejs,
  },
  {
    name: 'git',
    icon: git,
  },
  {
    name: 'figma',
    icon: figma,
  },
  {
    name: 'docker',
    icon: docker,
  },
];

const experiences = [
  {
    title: 'Front-End Web and React Developer',
    company_name: 'Study Independent | Dicoding',
    latest: true,
    icon: dicoding,
    iconBg: '#E6DEDD',
    date: 'Aug 2022 - Des 2022',
    points: [
      'Dicoding Learned and applied a wide range of web front-end skills, from basic to expert level, while collaborating with a team.',
      'As a team leader, I was responsible for generating ideas, developing both frontend and backend components, and presenting projects.',
    ],
  },
  {
    title: 'Full-Stack Web Developer',
    company_name: 'Scientific Writing',
    icon: gundar,
    iconBg: '#383E56',
    date: 'Apr 2022 - Aud 2022',
    points: [
      'Universitas Gunadarma Developed an online invitation web application using the MERN stack technology.',
      'Implemented a payment gateway from midtrans to facilitate secure and seamless transactions within the application.',
      'Implementing responsive design and ensuring cross-browser compatibility.',
    ],
  },
  {
    title: 'Part-time Front-End Developer',
    company_name: 'Bintang Studio',
    icon: bintang,
    iconBg: '#FFFFFF',
    date: '2021-2022',
    points: [
      'Developing and maintaining web applications using React.js and other related technologies.',
      'Created a visually appealing photography service website using ReactJS.',
      'Implementing responsive design and ensuring cross-browser compatibility.',
    ],
  },
];

const certificates = [
  {
    title: 'Weekly Class REACT.JS WEB DEVELOPMENT',
    name: 'Google Developer Student Clubs',
    link: 'https://ibb.co/54s1VtD',
    image:
      'https://studentorgs.engineering.asu.edu/wp-content/uploads/sites/35/2023/01/GDSC-Crop.png',
  },
  {
    title: 'Weekly Class Web Development 2022',
    name: 'Google Developer Student Clubs',
    link: 'https://ibb.co/gSz7PYh',
    image:
      'https://studentorgs.engineering.asu.edu/wp-content/uploads/sites/35/2023/01/GDSC-Crop.png',
  },
  {
    title: 'Weekly Class UI/UX Design 2022',
    name: 'Google Developer Student Clubs',
    link: 'https://ibb.co/gw1Lsbj',
    image:
      'https://studentorgs.engineering.asu.edu/wp-content/uploads/sites/35/2023/01/GDSC-Crop.png',
  },
  {
    title: 'Weekly Class Android 2022',
    name: 'Google Developer Student Clubs',
    link: 'https://ibb.co/qjjCC2j',
    image:
      'https://studentorgs.engineering.asu.edu/wp-content/uploads/sites/35/2023/01/GDSC-Crop.png',
  },
  {
    title: 'Memulai Dasar Pemrograman untuk Menjadi Pengembang Software',
    name: 'Dicoding Indonesia',
    link: 'https://www.dicoding.com/certificates/JMZVG8GDJZN9',
    image:
      'https://dicoding-web-img.sgp1.cdn.digitaloceanspaces.com/original/jobs/dos:lowongan_dicoding_dicoding_indonesia_010322094804.png',
  },
  {
    title: 'Belajar Dasar Pemrograman Web',
    name: 'Dicoding Indonesia',
    link: 'https://www.dicoding.com/certificates/2VX31765JZYQ',
    image:
      'https://dicoding-web-img.sgp1.cdn.digitaloceanspaces.com/original/jobs/dos:lowongan_dicoding_dicoding_indonesia_010322094804.png',
  },
  {
    title: 'Belajar Dasar Git dengan GitHub',
    name: 'Dicoding Indonesia',
    link: 'https://www.dicoding.com/certificates/6RPND01Y5Z2M',
    image:
      'https://dicoding-web-img.sgp1.cdn.digitaloceanspaces.com/original/jobs/dos:lowongan_dicoding_dicoding_indonesia_010322094804.png',
  },
  {
    title: 'Belajar Membuat Front-End Web untuk Pemula',
    name: 'Dicoding Indonesia',
    link: 'https://www.dicoding.com/certificates/L4PQ40MQOXO1',
    image:
      'https://dicoding-web-img.sgp1.cdn.digitaloceanspaces.com/original/jobs/dos:lowongan_dicoding_dicoding_indonesia_010322094804.png',
  },
  {
    title: 'Belajar Fundamental Front-End Web Development',
    name: 'Dicoding Indonesia',
    link: 'https://www.dicoding.com/certificates/07Z6RO8M2PQR',
    image:
      'https://dicoding-web-img.sgp1.cdn.digitaloceanspaces.com/original/jobs/dos:lowongan_dicoding_dicoding_indonesia_010322094804.png',
  },
  {
    title: 'Belajar Membuat Aplikasi Web dengan React',
    name: 'Dicoding Indonesia',
    link: 'https://www.dicoding.com/certificates/6RPNDM399Z2M',
    image:
      'https://dicoding-web-img.sgp1.cdn.digitaloceanspaces.com/original/jobs/dos:lowongan_dicoding_dicoding_indonesia_010322094804.png',
  },
  {
    title: 'Belajar Fundamental Aplikasi Web dengan React',
    name: 'Dicoding Indonesia',
    link: 'https://www.dicoding.com/certificates/L4PQ6M6MVPO1',
    image:
      'https://dicoding-web-img.sgp1.cdn.digitaloceanspaces.com/original/jobs/dos:lowongan_dicoding_dicoding_indonesia_010322094804.png',
  },
  {
    title: 'Menjadi Front-End Web Developer Expert',
    name: 'Dicoding Indonesia',
    link: 'https://www.dicoding.com/certificates/6RPN643G5P2M',
    image:
      'https://dicoding-web-img.sgp1.cdn.digitaloceanspaces.com/original/jobs/dos:lowongan_dicoding_dicoding_indonesia_010322094804.png',
  },
  {
    title: 'Menjadi React Web Developer Expert',
    name: 'Dicoding Indonesia',
    link: 'https://www.dicoding.com/certificates/JMZVN792OPN9',
    image:
      'https://dicoding-web-img.sgp1.cdn.digitaloceanspaces.com/original/jobs/dos:lowongan_dicoding_dicoding_indonesia_010322094804.png',
  },
  {
    title: 'Junior Web Programming',
    name: 'Badan Nasional Sertifikasi Profesi',
    link: 'https://ibb.co/PrYnjXL',
    image: 'https://cdn2.tstatic.net/tribunnewswiki/foto/bank/images/bnsps.jpg',
  },
];

const projects = [
  {
    id: 1,
    name: 'Nafas',
    desc: 'Web-based platform that allows users to search, buy, and manage blog, providing a convenient and efficient solution for plants your needs.',
    image: nafas,
    link: 'https://nafas-mu.vercel.app/',
    cat: 'Website Development',
  },
  {
    id: 2,
    name: 'EUndang',
    desc: 'A web application that allows users to order invitation templates, view interesting themes, and be able to order with a paymentgateway.',
    image: eundang,
    link: 'https://github.com/yar-24/admin-eundang',
    cat: 'Website Development',
  },
  {
    id: 3,
    name: 'Bintang photo',
    desc: 'Booking platform and photography services available at Bintang photo studio Yogyakarta.',
    image: bintangphoto,
    link: 'https://github.com/yar-24/bintangphoto',
    cat: 'Website Development',
  },
  {
    id: 4,
    name: 'Mendelof',
    desc: 'Mendelof is an application to classify leaves on coffee plants by applying the CNN (Convolutional Neural Network) method. Mendelof also contains ways to treat coffee plants that are attacked by disease.',
    image: mendelof,
    link: 'https://mendelof.vercel.app/',
    cat: 'Website Development',
  },
  {
    id: 5,
    name: 'SablonKuy',
    desc: 'This application project was created using react native and implemented payment gateway and rajaongkir as APIs to calculate shipping. ',
    image: sablonkuy,
    link: 'https://github.com/yar-24/sablon24App',
    cat: 'Mobile Development',
  },
  {
    id: 6,
    name: 'Dicoding Forum',
    desc: 'This application is a forum for programmers and as a task of dicoding.',
    image: forum,
    link: 'https://forum-app-yar.vercel.app/',
    cat: 'Website Development',
  },
];

const expertise = [
  {
    icon: monitor,
    title: 'Frontend Dev',
    subtitle: 'ReactJS & React Native',
    desc: 'Passionate about FE. Over 3 years of development experience in HTML, CSS, JS , TS, ReactJs & React Native frameworks.  ',
    color: '#df058d',
  },
  {
    icon: node,
    title: 'Backend Dev',
    subtitle: 'NodeJS & ExpressJS',
    desc: 'Experienced in BE. Used node and ExpressJS development environments and provides a variety of features  and functionality for developing and running applications with a RESTful API.',
    color: '#2c49d8',
  },
  {
    icon: react,
    title: 'Fullstack Dev',
    subtitle: 'MERN & NextJS',
    desc: 'Skilled in developing Fullstack apps using the MERN stack and NextJS framework.',
    color: '#ff6b2b',
  },
];

const katamereka = [
  {
    id: 1,
    word: 'Since 2020 Yare has been responsible for the development of our website which has been instrumental to the growth of our company. Even while working remotely he’s been highly responsive, organized and strategic in his thinking. In addition to staying on top of day-to-day site changes and builds, he’s provided us with great advice to stay on top of the current changes in web technologies. He’s also implemented effective project management and Q&A processes. As a result Yare has been an highly valued and impactful member of our team.',
    name: 'Fathur Azhari',
    position: 'Founder at Bintang Foto Studio',
    bgColor: '#bc60fb',
  },
  {
    id: 2,
    word: 'Yare is AMAZING! If you have any doubt about hiring him, ask me – I am really impressed by this guy!',
    name: 'Amar Andra',
    position: 'Client',
    bgColor: '#0067dc',
  },
  {
    id: 3,
    word: 'Yare is one of the best professionals that we have known in web development skills. Between his skills you can find good communication and accuracy with the planning in complex projects.',
    name: 'Yunita Ayu',
    position: 'Mentor Dicoding',
    bgColor: '#923fe6',
  },
];

export {
  services,
  technologies,
  experiences,
  certificates,
  projects,
  expertise,
  katamereka,
};
